...
> ANSWER:       
The str_subset(macbeth, "  MACBETH") finds a pattern that shows how many times Macbeth spoke in the play. Adding the header displays the first 6 instances. 
>          
head(str_which(macbeth, "  MACBETH")) returns the first six indices of the haystack in which the needles were in the  found.
>          
head(str_detect(macbeth, "  MACBETH")) returns the first six logical vectors. It returns true if the syntax is the same as the one in the function, and false if it is not.
...
> ANSWER: 
head(str_subset(macbeth, "MACBETH\\.")) will display the first 6 instances where the character Macbeth is accompanied by a period.          
>head(str_subset(macbeth, "MACBETH.")) will display the first instances where there is Macbeth, regardless of what comes after.
...
> ANSWER:          
head(str_subset(macbeth, "MAC[B-Z]")): this pattern will match any lines that contain MAC followed by any capital letter other than A.   

>head(str_subset(macbeth,"MAC[B|Z]")): this pattern will match any lines that contain either MACB or MACD.

>head(str_subset(macbeth, "^MAC[B-Z]")): this pattern anchors a pattern that has no space between MAC at the beginning of a piece of text, followed by any capital letter other than A.
