...
```{r}
Pitching1 <- select(Pitching, playerID, W, SO)

Pitching2<- aggregate(cbind(W, SO) ~ playerID, data = Pitching1, sum)

Pitching3<- filter(Pitching2, W > 300 & SO > 3000)
Pitching3
...
> ANSWER: ryanno01 had the most accumulated strikeouts of 5714. The most strikeouts he had in one season is 383.	

```{r}
#Maximum strikeout
Pitching4<- filter(Pitching3, SO== max(Pitching3$SO))
Pitching4

#player ryanno01 games only
Pitching5<- filter(Pitching, playerID == "ryanno01")
max(Pitching5$SO)
```
...
> ANSWER: There is a moderate positive assosiation between the number of inspections and the median score.  
 

```{r}
data("Violations")
Violations1<- Violations %>% drop_na()
```


```{r}
Violation2 <-Violations1 %>%
  filter(boro == "MANHATTAN") %>%
  select(zipcode,dba,score,)%>%
  group_by(zipcode,dba) %>%
  summarise(num_inspections=n(), med_score=median(score))

g <- ggplot(data = Violation2, aes(y = med_score, x = num_inspections)) + ggtitle("Number of inspections vs the median score") + ylab("median score") + xlab("no. of inspections")
g + geom_point(size = 1)
...
```{r}
#filter(Violation2, num_inspections>105)

context <- tribble(
  ~num_inspections, ~med_score, ~label,
  90, 30, "Starbucks")

g + geom_point(size = 1) +
  geom_text(
    data = context, 
    aes(y = med_score, label = label, color = label)
  ) + 
  geom_curve(
    x = 90, xend = 110, y = 30, yend = 10.5, 
    arrow = arrow(length = unit(0.2, "cm")), curvature = -0.5
  ) + 
  scale_color_manual(
    guide = FALSE, 
    values = c("black", "#b2d7e9", "darkgray")
  ) + 
  ylim(0, 100) + xlim(0, 115)
