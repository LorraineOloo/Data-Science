...
```{r}
select(flights_joined2 <- flights_joined %>% 
  
  mutate(distance_km = distance*1.6),name, flight, arr_delay,distance_km) 
...
```{r}
flights_joined3<- flights_joined2 %>%
  select(name, flight,arr_delay,distance_km) %>%
  filter(distance_km<500 & is.na(arr_delay)==FALSE)
...
babynames2<- babynames::babynames %>%
...
> ANSWER: in the sex column, one gender is listed first, the next gender is listed next instead of two different columns for the M and F.
...
> ANSWER: The filter function used in the making of BabyWide where both F and M were > 10000
