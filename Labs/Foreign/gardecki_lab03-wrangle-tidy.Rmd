...
```{r}
coverage <- read_csv("C:/Users/Lorraine/Dropbox/Data Science/Data-Science/data/healthcare_coverage.txt")
spending <- read_csv("C:/Users/Lorraine/Dropbox/Data Science/Data-Science/data/healthcare_spending.txt")

```

```{r, message = FALSE}
#coverage <- read_csv("/Users/emmagardecki/Documents/junior yr/stat231/stat231g/data/healthcare_coverage.txt")
#spending <- read_csv("/Users/emmagardecki/Documents/junior yr/stat231/stat231g/data/healthcare_spending.txt")
```

```{r}
coverage1 <- mutate( coverage, .before = "Location", num = row_number(  c( 1:52 ) )) 
spending1 <- mutate( spending, .before = "Location", num = row_number( c( 1:52 )))
```
...
> RESPONSE: The spending dataset covers the years 1991 to 2014, while the coverage dataset covers the years 2013 to 2016. Therefore, the overlapping data only includes the years 2013 and 2014. 

```{r}
#creates combined dataframe and cleans it up to feature only location, and yrs 2013, 2014
spend_cov <- merge( spending1, coverage1, by = "num")
spend_cov <- spend_cov %>%
  select( starts_with( c( "Location","2013", "2014"))) 
spend_cov$Location.x<- NULL
spend_cov <- rename( spend_cov, "Location" = Location.y )
...
```{r}
#incase you don't know the exact column you can use the filter function ~ Lorraine
spend_cov <- spend_cov[ -c( 1, 10 ), ] 
```
...
> RESPONSE: ggplot(spend_cov, aes(log(total health spending), log(employer))) + geom_point(color = year). 

What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: We need to make year a variable and merge all of the matching varibales together (aka 2013 and 2014 employer, etc.)
...
#separates spend_cov dataset into 2013 and 2014 
spend_cov2013 <- spend_cov %>%
  select( starts_with( c( "Location","x2013"))) 

spend_cov2014 <- spend_cov %>%
  select( starts_with( c( "Location", "x2014" )))
```

```{r}
#adds year column and cleans all the var names
spend_cov2013$year <- "2013"
spend_cov2013 <- 
  rename( spend_cov2013, c("spending" = x2013_total_health_spending, "employer" = x2013_employer, "non_group" = x2013_non_group, "medicaid" = x2013_medicaid, "medicare" = x2013_medicare, "other_public" = x2013_other_public, "uninsured" = x2013_uninsured, "total" = x2013_total))

spend_cov2014$year <- "2014"
spend_cov2014 <- 
  rename( spend_cov2014, c("spending" = x2014_total_health_spending, "employer" = x2014_employer, "non_group" = x2014_non_group, "medicaid" = x2014_medicaid, "medicare" = x2014_medicare, "other_public" = x2014_other_public, "uninsured" = x2014_uninsured, "total" = x2014_total))

#merges datasets back together now with double the cases 
spend_cov2 <- rbind( spend_cov2013, spend_cov2014)
```

```{r}
ggplot( spend_cov2, aes( log(spending), log(employer))) + geom_point( aes( color = year ))
```
...
> RESPONSE: In both scatterplots below there is still a strong positive relationship. However, these plots are more densely populated near (0, 0) and feature wide gaps as the total population increases (and thus the spending as well). 

```{r}
 spend_cov2 <- rename( spend_cov2, "total_pop" = total)

ggplot( spend_cov2, aes( total_pop, employer)) + geom_point( aes( color = year))
ggplot( spend_cov2, aes( total_pop, spending)) + geom_point( aes( color = year))
```
