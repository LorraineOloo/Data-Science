...
> ANSWER:
https://en.wikisource.org/wiki/ and then the name of the poem, separating the texts with an undersore
...
```{r}
poem_urls <- poem_table_all %>%
 mutate(first_line = str_replace_all(first_line_often_used_as_title, " ", "_"), url = paste0("https://en.wikisource.org/wiki/", first_line))
```
...
```{r}
# identify webpage URL you want to scrape
Bee_url <- "https://en.wikisource.org/wiki/A_Bee_his_burnished_Carriage"
...
all_poems <- data.frame(name = poem_urls$first_line
                        , text = ""
